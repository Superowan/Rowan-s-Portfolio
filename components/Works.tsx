...
const WORK_ITEMS: WorkItem[] = [
  { id: 1, title: 'Professional Experience', icon: '💼', bgColor: '#FFFFFF' },
  { id: 2, title: 'Strategic Competition', icon: '🏆', bgColor: '#FFFFFF' },
  { id: 3, title: 'Base Solution', icon: 'https://i.postimg.cc/76jWQRGr/20260126-175928.png', bgColor: '#FFFFFF' },
  { id: 4, title: 'AI Exploration', icon: '🤖', bgColor: '#FFFFFF' },
  { id: 5, title: 'Visual Creative', icon: '🎨', bgColor: '#FFFFFF' },
  { id: 6, title: 'Insights & Media', icon: '📊', bgColor: '#FFFFFF' },
];
...
const Works: React.FC<WorksProps> = ({ onSelect }) => {
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 gap-y-12 gap-x-8 md:gap-y-16 md:gap-x-12 max-w-4xl mx-auto px-4">
      {WORK_ITEMS.map((item) => {
        const isImageUrl = item.icon.startsWith('http');
          ...
          >
            ...
            >
              {isImageUrl ? (
                ...
                <span className="text-6xl md:text-7xl transform transition-all duration-500 group-hover:scale-110 grayscale group-hover:grayscale-0 opacity-40 group-hover:opacity-100">
                  {item.icon}
                </span>
              )}
            </div>
